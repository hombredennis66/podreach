...
class AuthApiClient {
  ...
  async updateConfig<T = any>(partial: Record<string, unknown>): Promise<T> {
    // Extract Spotify API credentials - these go to a separate endpoint
    const { client_id, client_secret, ...configPayload } = partial;

    // If Spotify API credentials are included, save them separately
    if (client_id !== undefined || client_secret !== undefined) {
      await this.apiClient.put("/credentials/spotify_api_config", {
        client_id: client_id ?? "",
        client_secret: client_secret ?? "",
      });
    }

    // Save the rest to the main config endpoint
    const response = await this.apiClient.put<T>("/config", configPayload);
    return response.data;
  }
}
