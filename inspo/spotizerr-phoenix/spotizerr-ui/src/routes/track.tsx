...
import type { LibrespotTrackType } from "@/types/librespot";
...
export const Track = () => {
  ...
  const [track, setTrack] = useState<LibrespotTrackType | null>(null);
  ...
  useEffect(() => {
    const fetchTrack = async () => {
      if (!trackId) return;
      try {
        const response = await apiClient.get<LibrespotTrackType>(`/track/info?id=${trackId}`);
