...
export function DownloadsTab({ config, isLoading }: DownloadsTabProps) {
  ...
  const onSubmit: SubmitHandler<DownloadSettings> = (data) => {
    ...
    mutation.mutate({
      ...
      realTimeMultiplier: Math.max(1, Number(data.realTimeMultiplier) || 1),
    });
  };
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-content-secondary dark:text-content-secondary-dark">1–10</span>
          </div>
          <input
