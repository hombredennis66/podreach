# Spotizerr-Phoenix API Documentation
...
#### `GET /episode/download/{episode_id}`
...
Episode-aware task responses may also include:

- `final_path`: exact downloaded audio path reported by the Phoenix worker
- `terminal_reason`: normalized terminal failure reason when a task stalls or fails
...
_This documentation covers all endpoints discovered in the Spotizerr-Phoenix routes directory. The API is designed for high-throughput music downloading with comprehensive monitoring and management capabilities._
