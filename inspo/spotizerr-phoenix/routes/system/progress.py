...
from routes.utils.episode_support import extract_final_path, extract_terminal_reason
...
def _resolve_task_artifacts(task_info: dict, last_status: dict | None, last_line: dict | None) -> tuple[str, str]:
    final_path = (
        task_info.get("final_path", "")
        or extract_final_path(last_line)
        or extract_final_path(last_status)
    )
    terminal_reason = (
        task_info.get("terminal_reason", "")
        or (last_status or {}).get("terminal_reason", "")
        or extract_terminal_reason(last_line)
        or extract_terminal_reason(last_status)
    )
    return final_path, terminal_reason
...
def build_task_object_from_callback(
    ...
    try:
        ...
        if not task_info:
            return None
        final_path, terminal_reason = _resolve_task_artifacts(task_info, None, callback_data)
        return {
            "task_id": task_id,
...
async def transform_callback_to_task_format(task_id: str, event_data: dict) -> dict:
    ...
    try:
        ...
        task_object = {
            ...
            "artist": task_info.get("artist", ""),
            "created_at": task_info.get("created_at"),
            "final_path": _resolve_task_artifacts(task_info, None, callback_data)[0],
            "terminal_reason": _resolve_task_artifacts(task_info, None, callback_data)[1],
        }
...
def _build_task_response(
    ...
    final_path, terminal_reason = _resolve_task_artifacts(task_info, last_status, last_line_content)
    if final_path:
        task_response["final_path"] = final_path
    if terminal_reason:
        task_response["terminal_reason"] = terminal_reason
