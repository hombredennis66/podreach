...
def download_playlist(
    ...
    try:
        if not main:
            raise ValueError("Spotify account name is required for Spotify downloads.")
        ...
        if service == "spotify":
            if fallback:  # Fallback is a Deezer account name for a Spotify URL
                ...
                try:
                    ...
                    if not deezer_fallback_creds:
                        raise ValueError(
                            f"Deezer credentials not found for account '{fallback}'."
                        )
                    ...
                    try:
                        ...
                        if not spotify_main_creds:
                            raise ValueError(
                                f"Spotify credentials not found for account '{main}'."
                            )
                        ...
                        logger.info(
                            "Using Spotify device info for account '%s' in playlist download (%s)",
                            main,
            ...
            if not deezer_main_creds:
                raise ValueError(f"Deezer credentials not found for account '{main}'.")
            arl = deezer_main_creds.get("arl")
            if not arl:
                raise ValueError(f"ARL not found for Deezer account '{main}'.")

            config = get_config_params() or {}
            active_spotify = config.get("spotify")
            dl = DeeLogin(
                ...
                spotify_credentials_path=(
                    str(get_spotify_blob_path(active_spotify))
                    if active_spotify
                    else None
                ),
            )
