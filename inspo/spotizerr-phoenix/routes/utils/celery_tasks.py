...
def _update_task_info_fields(task_id: str, task_info: dict, **updates) -> dict:
    changed = False
...
class ProgressTrackingTask(Task):
    ...
    def progress_callback(self, progress_data):
        ...
        self._sync_task_artifacts(task_id, progress_data, task_info)
        progress_data["raw_callback"] = raw_callback_data
        store_task_status(task_id, progress_data)

        abort_reason = progress_data.get("circuit_breaker_abort", "")
        if abort_reason:
            raise RuntimeError(abort_reason)

    def _sync_task_artifacts(self, task_id, data, task_info):
        """Persist exact file paths and terminal reasons for downstream clients."""
        final_path = extract_final_path(data)
        if final_path:
            data["final_path"] = final_path
            _update_task_info_fields(task_id, task_info, final_path=final_path)

        terminal_reason = extract_terminal_reason(data)
        if terminal_reason:
            data["terminal_reason"] = terminal_reason
            status = extract_status_info(data).get("status", data.get("status"))
            if status == "error" or data.get("circuit_breaker_abort"):
                _update_task_info_fields(
                    task_id,
                    task_info,
                ...
                _update_task_info_fields(task_id, task_info, last_error=terminal_reason)
    ...
    def _handle_downloading(self, task_id, data, task_info):
        ...
        if track_name == "Unknown" or not artist_name:
            track_name, fallback_artist = extract_episode_identity(data)
            if not artist_name:
    ...
    def _handle_retrying(self, task_id, data, task_info):
        ...
        if not song or not artist or song == "Unknown" or artist == "Unknown":
            song, artist = extract_episode_identity(data)

        retry_count = data.get("retry_count", status_info.get("retry_count", 0))
        seconds_left = data.get("seconds_left", status_info.get("seconds_left", 0))
        error = (
            data.get("error")
            or data.get("message")
            or extract_terminal_reason(data)
            or "Unknown error"
        )
        ...
        if task_info.get("download_type") == "episode":
            config_params = get_config_params()
            circuit_state, abort_reason = update_episode_retry_state(
                task_info,
                data,
                repeat_limit=int(config_params.get("episodeRetryCircuitRepeats", 6) or 6),
                seconds_limit=int(config_params.get("episodeRetryCircuitSeconds", 90) or 90),
            )
            task_info["episode_retry_circuit"] = circuit_state
            if abort_reason:
                task_info["terminal_reason"] = abort_reason
                data["terminal_reason"] = abort_reason
                data["circuit_breaker_abort"] = abort_reason
                logger.error("Task %s episode retry circuit opened: %s", task_id, abort_reason)
    ...
    def _handle_error(self, task_id, data, task_info):
        ...
        task_info["terminal_reason"] = message
        task_info["last_error"] = message
        store_task_info(task_id, task_info)

        # Set status and error message
        # data["status"] = ProgressState.ERROR
        data["error"] = message
        data["terminal_reason"] = message
    ...
    def _handle_done(self, task_id, data, task_info):
        ...
        if task_info.get("download_type") == "episode" and not content_type:
            content_type = "episode"
        album = data.get("album", "")
        artist = data.get("artist", "")
        song = data.get("song", "")
        if content_type == "episode" and (not song or not artist):
            song, artist = extract_episode_identity(data)

        final_path = extract_final_path(data)
        if final_path:
            task_info["final_path"] = final_path
            store_task_info(task_id, task_info)
            data["final_path"] = final_path
        ...
        if content_type == "track":
            ...
            if content_type == "episode":
                logger.info(f"Task {task_id} completed: EPISODE '{song}' by {artist}")
            else:
                logger.info(f"Task {task_id} completed: {content_type.upper()}")
            # data["status"] = ProgressState.COMPLETE
...
def task_failure_handler(
    ...
    try:
        ...
        last_status = get_last_task_status(task_id)
        if task_info:
            task_info["terminal_reason"] = str(exception)
...
@celery_app.task(
    bind=True, base=ProgressTrackingTask, name="download_episode", queue="downloads"
)
def download_episode(self, **task_data):
    """Task to download a podcast episode."""
    try:
        logger.info(
            f"Processing episode download task: {task_data.get('name', 'Unknown')}"
        )
        from routes.utils.episode import download_episode as download_episode_func
        ...
        final_path = download_episode_func(
            url=url,
            main=config_params.get("spotify", ""),
            quality=config_params.get("spotifyQuality", "NORMAL"),
            real_time=real_time,
            ...
            bitrate=bitrate,
            real_time_multiplier=real_time_multiplier,
            _is_celery_task_execution=True,
        )

        resolved_path = str(Path(final_path).resolve())
        task_info = get_task_info(self.request.id)
        if task_info:
            task_info["final_path"] = resolved_path
            task_info.pop("terminal_reason", None)
            store_task_info(self.request.id, task_info)
            trigger_sse_event(self.request.id, "episode_final_path")
        ...
        task_info = get_task_info(self.request.id)
        if task_info:
            task_info["terminal_reason"] = str(e)
            task_info["last_error"] = str(e)
            store_task_info(self.request.id, task_info)
        logger.error(f"Error in download_episode task: {e}")
        traceback.print_exc()
        raise
