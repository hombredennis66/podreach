...
SPOTIFY_EPISODE_URL = "https://open.spotify.com/episode/5jJKjhUY37MvTCp4sCXjPj"
...
EPISODE_ID = SPOTIFY_EPISODE_URL.split('/')[-1].split('?')[0]
...
def test_episode_download_and_verify_file(base_url, task_waiter, reset_config):
    """Tests downloading a podcast episode and verifies that audio lands on disk."""
    print("\n--- Testing Spotify-only episode download and verifying files ---")
    config_payload = {
        "service": "spotify",
        "fallback": False,
        "realTime": False,
    ...
    response = requests.get(f"{base_url}/episode/download/{EPISODE_ID}")
    assert response.status_code == 202
    task_id = response.json()["task_id"]

    final_status = task_waiter(task_id, timeout=1200)
    status = final_status.get("status") or final_status.get("status_info", {}).get("status")
    assert status in {"done", "complete", "completed"}, f"Episode task failed: {final_status}"

    downloaded_files = get_downloaded_files()
    assert len(downloaded_files) >= 1, "Episode download completed but no audio file was found."
