...
def wait_for_task(base_url, task_id, timeout=600):
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            status_info = last_status.get("status_info", {}) if isinstance(last_status, dict) else {}
            status = last_status.get("status") or status_info.get("status")
            
            # More verbose logging for debugging during tests
            message = last_status.get('message') or status_info.get('message') or status_info.get('error', '')
            track_obj = last_status.get('track', {}) if isinstance(last_status, dict) else {}
            if isinstance(track_obj, dict):
                track = track_obj.get('title', '')
            else:
            ...
            if status in ["done", "complete", "completed", "ERROR", "error", "failed", "cancelled", "ERROR_RETRIED", "ERROR_AUTO_CLEANED"]:
