# Spotizerr-Phoenix Backend Tests

This directory contains automated tests for the Spotizerr-Phoenix backend API.

## Prerequisites

1.  **Running Backend**: Ensure the Spotizerr-Phoenix Flask application is running and accessible at `http://localhost:7171`. You can start it with `python app.py`.
...
3.  **Credentials**: These tests require valid Spotify and Deezer credentials. Create a file named `.env` in the root directory of the project (`spotizerr-phoenix`) and add your credentials to it. The tests will load this file automatically.
...
To run all tests, navigate to the root directory of the project (`spotizerr-phoenix`) and run `pytest`:
