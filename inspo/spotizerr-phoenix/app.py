...
from routes.content.episode import router as episode_router  # noqa: E402
...
async def lifespan(app: FastAPI):
    ...
    enable_device_backfill = os.getenv(
        "SPOTIFY_DEVICE_INFO_BACKFILL", "true"
    ).lower() in ("1", "true", "yes", "on")
    if enable_device_backfill:
        try:
            from routes.utils.credentials import backfill_spotify_device_info

            backfill_spotify_device_info()
        except Exception as e:
            logging.getLogger(__name__).error(
                "Spotify device info backfill failed during startup: %s",
                e,
                exc_info=True,
            )
    else:
        logging.getLogger(__name__).info(
            "SPOTIFY_DEVICE_INFO_BACKFILL is disabled; skipping device info backfill."
        )
...
def create_app():
    ...
    app.include_router(episode_router, prefix="/api/episode", tags=["episode"])
